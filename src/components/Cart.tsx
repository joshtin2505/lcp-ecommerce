...
function Cart() {
  const {
    ...
    removeFromCart,
    decreaseQuantity,
  } = useCart() // destructuracion de las funciones del hook useCart
  const [isClient, setIsClient] = useState(false) // estado para saber si el componente esta montado
  useEffect(() => {
    // efecto para saber si el componente esta montado
    setIsClient(true)
  }, [])
  const handelClose = () => {
  ...
  const asideClass = openCart
    ? "fixed h-screen top-0 right-0 py-5 cartContainer z-20 gap-3 pr-9 w-96  flex-col flex"
    : "hidden"
  return (
    <aside className={asideClass}>
      <header className="h-20 flex w-full justify-center items-center rounded-md shadow-md headerCart">
        ...
        <button
          className="absolute right-2 closeBtn rounded-full p-1 transition-all hover:scale-105"
          onClick={handelClose}
        >
          <BsX size={25} />
        </button>
          ...
          <>
              ...
              {cart.map((item) => {
                ...
                return (
                  <>
                    <li
                      key={id}
                      className="flex w-full justify-start items-center gap-2 relative itemInCartCard "
                    >
                      <figure className=" ">
                        <Image
                          className="aspect-[114/74] object-cover rounded transition-all hover:scale-110 hover:rounded"
                          src={imgSrc}
                          alt={title}
                          width={100}
                          height={100}
                      ...
                      <aside className="flex absolute right-3 quantityContainer rounded">
                        <button
                          className="outline-none btnDeleteItem rounded px-1 mr-[-2px] "
                          onClick={() => {
                            removeFromCart(item)
                          }}
                        >
                        ...
                        <button
                          className="outline-none disabled:hidden rounded px-1"
                          onClick={() => {
            ...
            <footer className=" w-72 absolute bottom-8 flex justify-center items-start gap-4 flex-col">
              ...
              <div className="flex justify-around items-center gap-3 w-full ">
                <button
                  onClick={() => clearCart()}
                  className="flex items-center justify-center gap-1 btnCleanCart rounded p-2 hover:scale-105 hover:shadow-md"
                >
                  Vaciar Carrito
                  <BsCartX />
                </button>
